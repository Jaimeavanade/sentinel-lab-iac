...
foreach ($sol in $solutions) {
  ...
  $contentSchemaVersion = $pkg.properties.contentSchemaVersion  # <-- CLAVE
  ...
  # Por si el catálogo no lo trae por alguna razón, fallamos con mensaje claro
  ...
  $installBody = @{
    properties = @{
      ...
      version             = $version
      contentSchemaVersion = $contentSchemaVersion   # <-- CLAVE (requerido por tu error)
    }
  ...
  if ($result.Content) { Write-Host "Install Response (raw): $($result.Content)" }
